...
import {
  containers, fields, templates, FIELD,
} from './registry';
 ...
 */
const renderElement = ({
  config, formik, value, error,
}) => {
  const {
    type, renderer, wrapAs, ...rest
  } = config;
...
const ElementRenderer = ({
  ...
  const [canShow, setCanShow] = useState(!showWhen);
  const [disabled, setDisabled] = useState(!!enabledWhen);
